...
export const Moves: {[moveid: string]: MoveData} = {
	...
	aeroswiftlash: {
		...
		// Granting Aero Veil status after move is used
		volatileStatus: 'aeroveil',
		condition: {
		  duration: 2,
		  onStart(pokemon) {
			this.add('-start', pokemon, 'Aero Swiftlash');
		  },
		  onModifyAccuracyPriority: 10,
		  onModifyAccuracy(accuracy, target, source, move) {
			if (typeof accuracy === 'number') {
			  this.add('-activate', source, 'move: Aero Swiftlash');
			  return accuracy * 0.75; // Represents an evasion increase effectively
			}
		  },
		  onEnd(pokemon) {
			this.add('-end', pokemon, 'Aero Swiftlash');
		  },
		},
		secondary: null,
		target: "normal",
		contestType: "Cool",
		desc: "Hits the target, then sharply raises the user's Speed by 2. Grants 'Aero Veil' for the next turn, increasing evasion for the first attack.",
		shortDesc: "Raises Speed by 2; next turn, increases evasion for the first attack.",
	  },
	aerowave: {
		num: 161,
		accuracy: 100,
	...
	bubblemirage: {
		...
		basePowerCallback(pokemon, target, move) {
			if (target.volatiles['confusion']) {
				this.debug('BP doubled from confusion');
				return move.basePower * 2;
			}
			return move.basePower;
		},
	...
	calculatedpounce: {
		...
		basePower: 100,
		category: "Physical",
		name: "Calculated Pounce",
		pp: 10,
		priority: 1,
		flags: {contact: 1, protect: 1, mirror: 1},
		onTry(source, target) {
			if (target.hp * 2 >= target.maxhp) {
				this.hint("Calculated Pounce only works if the enemy is below half HP");
				return false;
			}
		},
		target: "normal",
	...
	changetheweather: {
		...
		onHit(target, source) {
			...
			if (source.storedStats['spe'] > highestValue) {
				highestStat = "Speed";
				highestValue = source.storedStats['spe'];
			}
			if (source.storedStats['atk'] > highestValue) {
				highestStat = "Attack";
				highestValue = source.storedStats['atk'];
			}
			if (source.storedStats['spa'] > highestValue) {
				highestStat = "Special Attack";
				highestValue = source.storedStats['spa'];
			}
			if (source.storedStats['def'] > highestValue) {
				highestStat = "Defense";
				highestValue = source.storedStats['def'];
			}
			if (source.storedStats['spd'] > highestValue) {
				highestStat = "Special Defense";
				highestValue = source.storedStats['spd'];
			}
			...
			this.add('-message', `${source.name}'s highest stat is ${highestStat}, changing the weather!`);
		},
		secondary: null,
		target: "self",
		type: "Normal",
	...
	chaosstrike: {
		...
		basePower: 0,
		onModifyMove(move) {
			// Generate random base power between 50 and 150
			move.basePower = Math.floor(Math.random() * 101) + 50;
	
			// Generate a random type from the list of types
			...
			this.add('-message', `Type: ${move.type}, Base Power: ${move.basePower}`);
		},
		category: "Special",
		name: "Chaos Strike",
		pp: 10,
	...
	defiantstatement: {
		...
		priorityChargeCallback(pokemon) {
			this.hint("Registeel: There is nothing you can do to me. Do Your Worst.");
		},
		beforeMoveCallback(pokemon, target, move) {
			...
			if (damagedByTarget) {
				this.hint("Registeel: Revolting.")
			}
		},
		condition: {
			duration: 1,
			onStart(pokemon) {
				this.add('-singleturn', pokemon, 'move: Defiant Statement');
			},
		},
		secondary: null,
		target: "normal",
	...
	electrorush: {
		...
		basePowerCallback(pokemon, target) {
			let ratio = Math.floor(pokemon.getStat('spe') / target.getStat('spe'));
			if (!isFinite(ratio)) ratio = 0;
			const bp = [50, 70, 90, 120, 150][Math.min(ratio, 4)];
			this.debug('BP: ' + bp);
			return bp;
		},
		category: "Physical",
	...
	elegantguard: {
		...
		name: "Elegant Guard",
		pp: 10,
		type: "Normal", // Type can be adjusted to fit thematic elements
		category: "Status",
		priority: 0,
		flags: { snatch: 1 },
		selfSwitch: false,
		onTryHit(pokemon) {
			this.add('-message', `${pokemon.name} surrounds itself with a protective aura of elegance!`);
		},
		onHit(target) {
			this.add('-message', `${target.name} becomes more poised and graceful!`);
			target.addVolatile('elegantguard');
		},
		condition: {
			onStart(target) {
				this.add('-start', target, 'Elegant Guard');
			},
			onDamagingHit(damage, target, source, effect) {
				if (effect && effect.effectType === 'Move' && effect.category === 'Physical') {
					this.add('-ability', target, 'Elegant Guard');
					// Reflect 25% of the damage back to the attacker
					this.damage(damage / 2, source, target);
				}
			},
			onEnd(target) {
				this.add('-end', target, 'Elegant Guard');
			},
		},
		secondary: null,
		target: "self",
		contestType: "Beautiful",
		desc: "Raises the user's Defense by 1 stage and reflects 25% of physical damage back at the attacker for one turn.",
		shortDesc: "Boosts Defense by 1; reflects 25% of physical dmg back.",
		basePower: 0,
		accuracy: 0
	},
	...
	galacticnova: {
		...
		onHit(pokemon, source, target) {
		  if (this.randomChance(5, 10)) {
			this.add('-message', 'Galactic Nova resets the battlefield!');
			const clearEffects = ['reflect', 'lightscreen', 'auroraveil', 'safeguard', 'mist', 'electricterrain', 'grassyterrain', 'mistyterrain', 'psychicterrain', 'tailwind', 'hail', 'raindance', 'sandstorm', 'sunnyday'];
			for (const effect of clearEffects) {
			  source.side.removeSideCondition(effect);
			  source.side.foe.removeSideCondition(effect);
			}
			...
			for (const condition of sideConditions) {
				if (pokemon.hp && pokemon.side.removeSideCondition(condition)) {
					this.add('-sideend', source.side, this.dex.conditions.get(condition).name, '[from] move: Galactic Nova', '[of] ' + pokemon);
				}
			}
			this.add('-activate', source, 'move: Galactic Nova');
			let success = false;
		...
		target: "normal",
		contestType: "Cool",
		desc: "Deals damage with a 20% chance to reset the battlefield, removing terrain, weather, hazards, and stat changes.",
		shortDesc: "20% chance to reset battlefield conditions.",
	  },
	gastroacid: {
		num: 380,
		accuracy: 100,
		...
		onAfterMove(target, source, move) {
			if (!move.hasSheerForce) {
				if (target.hp && target.removeVolatile('leechseed')) {
					this.add('-end', target, 'Leech Seed', '[from] move: Rapid Spin', '[of] ' + target);
				}
				const sideConditions = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge', 'glowingspores'];
				for (const condition of sideConditions) {
					if (target.hp && target.side.removeSideCondition(condition)) {
						this.add('-sideend', target.side, this.dex.conditions.get(condition).name, '[from] move: Great Gust', '[of] ' + target);
					}
					if (source.hp && source.side.removeSideCondition(condition)) {
						this.add('-sideend', source.side, this.dex.conditions.get(condition).name, '[from] move: Great Gust', '[of] ' + source);
					}
				}
	...
	healingmist: {
		...
		onHit(target, source) {
			this.add('-activate', source, 'move: Healing Mist');
			this.hint("The Mist recovered your teams HP");
			let success = false;
			const allies = [...target.side.pokemon, ...target.side.allySide?.pokemon || []];
			for (const ally of allies) {
				if (ally.heal(target.baseMaxhp / 8)) success = true;
			}
			return success;
		},
		target: "allyTeam",
	...
	hiddenscript: {
		...
		onHit(pokemon) {
			...
			if (oldAbility) {
				this.add('-ability', pokemon, randomAbility, '[from] move: Hidden Script');
				return;
			}
			return oldAbility as false | null;
		},
	...
	martialartsmastery: {
		...
		onHit(target, source, move) {
		  const hitEffects = [
			{name: "Punch", basePower: 25, effect: null},
			{name: "Kick", basePower: 10, effect: () => { if (this.randomChance(1, 2)) this.boost({spe: -1}, target); }},
			{name: "Slash", basePower: 30, effect: null, critRatio: 3},
			{name: "Slap", basePower: 10, effect: () => { if (this.randomChance(1, 2)) this.boost({atk: -1}, target); }},
			{name: "Mega Punch", basePower: 60, effect: null},
			{name: "Jab", basePower: 20, effect: () => { if (this.randomChance(3, 10)) target.addVolatile('flinch'); }}
		  ];
	
		  let hitIndex = this.random(6); // Randomly select one of the hits
		  let hit = hitEffects[hitIndex];
		  if (hit.name === "Slash") {
			move.critRatio = hit.critRatio; // Apply high crit ratio for Slash
		  } else {
			delete move.critRatio; // Ensure crit ratio is only applied for Slash
		  }
	
		  // Set the move's properties based on the selected hit
		  move.basePower = hit.basePower;
	
		  // Display the effect being used
		  this.add('-message', `${source.name} used ${hit.name}!`);
	
		  // Apply the effect if it exists
		  if (hit.effect) hit.effect.call(this);
		},
		secondary: null,
		target: "normal",
		contestType: "Tough",
		desc: "Hits 2-5 times with each hit having a different effect. Effects include Punch, Kick (50% chance to lower opponent's Speed), Blast (special attack), Slap (50% chance to lower target's Attack), Mega Punch, and Jab (30% chance to make the target flinch).",
		shortDesc: "2-5 hits. Each has a different effect.",
	  },
	matblock: {
		num: 561,
		accuracy: true,
	...
	observation: {
		...
		onHit(target, source, move){
			...
			this.hint("Registeel: This is a great opportuity for slaughter.")
		},
		secondary: null,
		target: "self",
		type: "Normal",
	...
	prismaticsurge: {
		...
		onBasePower(basePower, pokemon, target) {
		  if (this.field.terrain) {
			return this.chainModify(1.3); // 30% power boost in terrain
		  }
		},
		onHit(target, source, move) {
		  ...
		  switch (terrain) {
			case 'electricterrain':
			  if (this.randomChance(5, 10)) target.trySetStatus('par', source);
			  break;
			case 'grassyterrain':
			  const damageDealt = source.lastDamage || 0;
			  source.heal(damageDealt / 3);
			  break;
			case 'mistyterrain':
			  if (this.randomChance(5, 10)) target.addVolatile('confusion', source);
			  break;
			case 'psychicterrain':
			  move.accuracy = true;
			  break;
		...
		target: "normal",
		contestType: "Cool",
		desc: "Absorbs terrain energy to boost power and gain additional effects. No terrain: sets Prismatic Terrain.",
		shortDesc: "Terrain-based effects. No terrain: boosts Steel moves.",
	  },
	protect: {
		num: 182,
		accuracy: true,
	...
	riftrupture: {
		...
		onPrepareHit(target, source, move) {
			...
			this.add('-anim', target, 'Hyper Beam', source);
		  },
		...
		target: "all",
		contestType: "Clever",
		desc: "Creates a rift that causes moves to fail 50% of the time for 2 turns. When the rift collapses, it damages all but the user.",
		shortDesc: "2-turn rift: 50% move fail. Ends with damage to all but user.",
	  },
	risingvoltage: {
		num: 804,
		accuracy: 100,
	...
	scorchingfury: {
		...
		secondary:  {},
		onHit(target, source) {
			const side = source.isAlly(source) ? source.side.foe : source.side;
			const firepledge = side.sideConditions['firepledge'];
			{
				side.addSideCondition('firepledge', target);
			}
			},
		target: "normal",
		type: "Fire",
	...
	scrapchuck: {
		...
		onBasePower(basePower, attacker, defender) {
			// Calculate additional damage based on entry hazards on the user's side
			const spikes = attacker.side.getSideCondition("spikes") ? 1 : 0;
			const toxicspikes = attacker.side.getSideCondition("toxicspikes") ? 1 : 0;
			const stealthrock = attacker.side.getSideCondition("stealthrock") ? 1 : 0; // Stealth Rock is a boolean condition
			const stickyweb = attacker.side.getSideCondition("stickyweb") ? 1 : 0;
			const glowingspores = attacker.side.getSideCondition("glowinspores") ? 1 : 0;
			const steelsurge = attacker.side.getSideCondition("gmaxsteelsurge") ? 1 : 0;
	
			// Calculate the bonus damage based on the number of hazards
			const bonusDamage = (spikes + toxicspikes + stealthrock + stickyweb + glowingspores + steelsurge) * 30;
	
			// Ensure the bonus damage doesn't exceed a certain limit
			const maxBonusDamage = 200;
			return basePower + Math.min(bonusDamage, maxBonusDamage);
		},
		onAfterHit(target, pokemon, move) {
			if (!move.hasSheerForce) {
				if (pokemon.hp && pokemon.removeVolatile('leechseed')) {
	...
	shellretreat: {
		...
		selfSwitch: true,
		onHit(target, source) {
			this.hint("it retreated into its shell")
			let success = false;
			target.cureStatus() || success; success = true;
			return success;
		},
		target: "self",
		type: "Bug",
	...
	soulshade: {
		...
		onHit(target) {
			...
			this.add("-activate", target, move.name, ppDeducted);
		},
		target: "normal",
		type: "Ghost",
		contestType: "Clever",
	...
	spikes: {
		...
		condition: {
			...
			onEntryHazard(pokemon) {
				if (!pokemon.isGrounded() || pokemon.hasItem('heavydutyboots' || pokemon.hasAbility('flutteringbug'))) return;
	...
	starlightveil: {
		...
		onHit(target, source, move) {
		  const effects = ['heal', 'boost', 'immunity'];
		  const selectedEffect = this.sample(effects);
	
		  switch (selectedEffect) {
			case 'heal':
			  this.add('-message', `${source.name}'s Starlight Veil heals its wounds!`);
			...
			case 'boost':
			  ...
			  this.add('-message', `${source.name}'s Starlight Veil boosts its ${stat}!`);
			  break;
			case 'immunity':
			  source.addVolatile('starryshield');
			  this.add('-message', `${source.name} is enveloped by a protective Starlight Veil, granting it immunity to damage this turn!`);
			  break;
		  }
		},
		condition: {
		  duration: 1,
		  onStart(pokemon) {
			this.add('-start', pokemon, 'Starlight Veil');
		  },
		  onTryHit(target, source, move) {
			if (target !== source && move.category !== 'Status') {
		...
		target: "normal",
		contestType: "Beautiful",
		desc: "Deals damage with a 30% chance to either heal the user, boost a random stat, or grant immunity to damage for a turn.",
		shortDesc: "30% chance of healing, boosting, or immunity.",
	  },
	stealthrock: {
		num: 446,
		accuracy: true,
	...
	toxicspikes: {
		...
		condition: {
			...
			onEntryHazard(pokemon) {
				...
				} else if (pokemon.hasType('Steel') || pokemon.hasItem('heavydutyboots') || pokemon.hasAbility('flutteringbug')) {
					return;
	...
	voltstream: {
		...
		secondary: {},
		onHit(target, source, move) {
			if(target.getStat('atk') >= target.getStat('spe')){
				target.trySetStatus('brn', target)
			}
			else {target.trySetStatus('par',target)}
